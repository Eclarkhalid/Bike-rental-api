...
export default function Product({
  ...
  const [numberPlate, setNumberPlate] = useState(existingGender || '');
  ...
  async function createProduct(ev) {
    ...
    const data = { title, description, price, details, images, category, brand, numberPlate, };
    if (_id) {
        ...
        <div className="grid grid-cols-2 items-center my-4">
          ...
          <div className="col-span-2">
            <textarea
              type="text"
              className="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-400 focus:ring focus:ring-primary-200 focus:ring-opacity-50 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500 border p-3"
              placeholder="Description about the Bicycle"
              rows={6}
              required
              value={description}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              value={numberPlate}
              onChange={ev => setNumberPlate(ev.target.value)}
            />
          </div>
        </div>
